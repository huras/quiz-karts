...
var allImagesLoaded = function allImagesLoaded() {// var mario = new Mario(marioSheet, this.context, new Vector2(20, 32));
  ...
  _createClass(GameEngine, [{
        ...
        for (var i = 0; i < this.kartRacers.length; i++) {
          ...
          if (collided) {
            ...
            this.gameState == gameStates.WRONG_ANSWER) {
              ...
              if (!this.studentResults[this.currentQuestion.question]) this.studentResults[this.currentQuestion.question] = [];
              ...
              if (respostaDoAluno == repostaCorreta) {
                ...
                this.gameState = gameStates.WRONG_ANSWER;
                setTimeout(function () {
                  if (_this3.gameState == gameStates.WRONG_ANSWER) _this3.gameState = gameStates.WAIT_ANSWER;
                }, 2000);
              }
            }
              ...
              case gameStates.LOAD_QUESTION:
                {
                  ...
                  if (this.currentQuestion) {
                    this.activeQuestions.splice(this.activeQuestions.indexOf(this.currentQuestion), 1);
                    this.currentQuestion = undefined;
                  }

                  if (this.activeQuestions.length > 0) {
                    this.currentQuestion = this.activeQuestions[Math.random() * this.activeQuestions.length | 0];
    ...
    value: function respawnRacers() {
      ...
      var kartRacer = function kartRacer() {
        var valorBandeira = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
      ...
      if (this.layout == "mobile") {
        this.kartRacers.push(kartRacer(6, {
          x: -401,
          y: this.getEstradaCoordinates(0.2)
        }));
        this.kartRacers.push(kartRacer(66, {
          x: -351 + 125,
          y: this.getEstradaCoordinates(0.4)
        }));
        this.kartRacers.push(kartRacer(999, {
          x: -201,
    ...
    value: function renderCeu() {
      this.ctx.drawImage(skybox, 0, 0, 1024, 720, 0, 0, canvas.width, canvas.height / 2);
    }
  }, {
    key: "renderNuvens",
    value: function renderNuvens() {
      ...
      this.nuvens = this.nuvens.map(function (nuvem) {
        _this5.ctx.drawImage(nuvem.img, 0, 0, nuvem.size.x, nuvem.size.y, nuvem.position.x, nuvem.position.y, nuvem.size.x * _this5.nuvemScale, nuvem.size.y * _this5.nuvemScale);
    ...
    value: function renderMontanhas() {
      this.ctx.drawImage(mountains, 0, 0, 1024, 215, 0, canvas.height / 2 - 100, canvas.width, 215);
    }
  }, {
    key: "renderRoad",
    value: function renderRoad() {
      ...
      this.ctx.drawImage(roadZebra, 0, 0, this.roadZebra.size.x, this.roadZebra.size.y, this.roadZebra.position.x, canvas.height / 2, this.roadZebra.size.x, this.roadZebra.size.y); // Faixa asfalto
      ...
      this.ctx.drawImage(roadLine, 0, 0, this.roadLine.size.x, this.roadLine.size.y, this.roadLine.position.x, canvas.height - 7 * canvas.height / 24, this.roadLine.size.x, this.roadLine.size.y);
    }
  }, {
    ...
    value: function renderJanelaFullscreen() {
      ...
      this.ctx.fillText("Toque aqui para", canvas.width / 2 + 5, canvas.height / 2 - 15, canvas.width);
      this.ctx.fillText("ativar a tela cheia", canvas.width / 2 + 5, canvas.height / 2 + 18, canvas.width);
    }
  }, {
    key: "renderRacers",
    value: function renderRacers() {
      ...
      if (this.gameState == gameStates.WAIT_ANSWER || this.gameState == gameStates.WRONG_ANSWER) {
        ...
        this.kartRacers.map(function (item, i, arr) {
          ...
          if (Math.abs(targetPositions[i] - item.position.x) < 8) speedToMove = targetPositions[i] - item.position.x;
        ...
        this.kartRacers.map(function (item, i, arr) {
          ...
          if (_this6.winnerKart == item) {
            targetPosition = canvas.width / 2 - item.size.x * _this6.racerScale / 2;
            var speedToMove = 4 * (targetPosition - item.position.x >= 0 ? 1 : -1);
            if (Math.abs(targetPosition - item.position.x) < 4) speedToMove = targetPosition - item.position.x;
      ...
      this.kartRacers.map(function (item) {
        _this6.racerScale = canvas.width > 768 ? 0.7 : _this6.racerMobileScale;

        _this6.ctx.drawImage(racer, 0, 0, item.size.x, item.size.y, item.position.x, item.position.y, item.size.x * _this6.racerScale, item.size.y * _this6.racerScale);

        if (showHitBoxes) {
          _this6.ctx.fillStyle = "rgba(255,0,0,0.36)";

          _this6.ctx.fillRect(item.position.x, item.position.y, item.size.x * _this6.racerScale, item.size.y * _this6.racerScale);
        }
      });
    }
  }, {
    ...
    value: function renderRacerFlags(question) {
      ...
      this.kartRacers.map(function (item, index) {
        ...
        _this7.ctx.fillText(item.valorBandeira, item.position.x + 165 * _this7.racerScale - (item.valorBandeira.toString().length - 1) * 8, item.position.y - (100 - item.size.y / 2) * _this7.racerScale, 140);
      });
    }
  }, {
    ...
    value: function renderMathOperation(question) {
      ...
      roundRect(canvasContext, rect.x, rect.y, rect.w, rect.h, 5, true, rect.borderSize); // Operation

      this.ctx.font = "36px sans-serif";
      this.ctx.fillStyle = "#FFFFFF";
      this.ctx.textBaseline = "middle";
      this.ctx.textAlign = "center";
      this.ctx.fillText(question.question, rect.x + rect.w / 2, rect.y + rect.h / 2 + rect.borderSize * 2, 1000); //Retorna valores ao normal
    ...
    value: function renderQuestionLabel2() {
      ...
      this.ctx.fillText("CLIQUE NO CARRO QUE APRESENTA", labelRect.x + labelRect.padding.x + 54, labelRect.y + labelRect.padding.y + fontSize, canvas.width - labelRect.x);
      this.ctx.fillText("O RESULTADO DA MULTIPLICAÇÂO.", labelRect.x + labelRect.padding.x + 54, labelRect.y + labelRect.padding.y + fontSize * 2, canvas.width - labelRect.x); //Circle with number
      // var circleRadius =
    ...
    value: function renderEnunciadoBtn() {
      ...
      this.ctx.beginPath();
      this.ctx.arc(this.enunciadoBtn.rect.x, this.enunciadoBtn.rect.y, this.enunciadoBtn.rect.w * 0.95, 0, 2 * Math.PI, false);
      ...
      this.ctx.drawImage(megafone, -this.enunciadoBtn.rect.w * 0.5 * animFactor, -this.enunciadoBtn.rect.h * 0.5 * animFactor, this.enunciadoBtn.rect.w * animFactor, this.enunciadoBtn.rect.h * animFactor);
      this.ctx.restore();
    }
  }, {
    key: "checkEnunciadoBtnClick",
    value: function checkEnunciadoBtnClick(x, y) {
      if (Physics.pointCircle({
        x: x,
        y: y
      }, this.enunciadoBtn.circle)) {
